...
const Challenges: React.FC<ChallengeProps> = ({ username }) => {
  ...
  const fetchChallenges = async () => {
    try {
      ...
      const activeChallengesResponse = await axios.get(
        'http://localhost:8080/Backend/Backend?reqID=4'
      );
      
      if (activeChallengesResponse.data.success && Array.isArray(activeChallengesResponse.data.challenges)) {
        const mappedProperties: Challenge[] = activeChallengesResponse.data.challenges.map((c: any) => ({
          id: c.id,
          title: c.title,
          description: c.description,
          utilityType: c.utility,
          reductionTarget: c.reductionTarget,
          startDate: c.startDate,
          endDate: c.endDate,
          rewardPoints: c.points
        }));
        setActiveChallenges(mappedProperties);
      }
      else {
      ...
      const userChallengesResponse = await axios.get(
        `http://localhost:8080/Backend/Backend?reqID=5&username=${username}`
      );
      
      if (userChallengesResponse.data.success && Array.isArray(userChallengesResponse.data.progress)) {
        const mappedProperties: Challenge[] = userChallengesResponse.data.progress.map((p: any) => ({
          id: p.id,
          title: p.title,
          ...
          status: p.status,
          dateCompleted: p.completeDate,
          pointsEarned: p.pointsEarned
        }));
        setUserChallenges(mappedProperties);
      }
      else {
        console.log("No challenges found");
      }
  ...
  const completeChallenge = async (challengeId: number) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/Backend/Backend', {
        reqID: 4, // COMPLETE_CHALLENGE
        challengeID: challengeId,
        username: username,
        // pointsEarned: selectedChallenge?.pointsEarned,   might add this but need to change the backend and check if it is null there since selected could be null
  ...
  const openChallengeModal = async (challenge: Challenge | UserChallenge) => {
    try {
      if (!('description' in challenge) || !('reductionTarget' in challenge)) {
        const challengeDetails = activeChallenges.find(c => c.id === challenge.id);
